local servers_path = 'plugins.android.lsp.servers'
local servers_dir = vim.fn.stdpath('config') .. '/lua/plugins/android/lsp/servers'
...
local function load_lsp(server)
    ...
    if ok then
        ...
        vim.notify("Error saat memuat lsp (android): " .. server, vim.log.levels.ERROR)
    end
end
